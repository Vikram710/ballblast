...
class Cannon{
    ...
    draw(ctx){
        ctx.drawImage(shooter,this.x,this.y,this.width,this.height);
    }
    moveLeft(){
        this.speed=-10;
    }
...
class Rock{
    ...
    hitBy(bullet){
        if(this.x+this.radius > bullet.x && this.x-this.radius < bullet.x+bullet.width
            && this.y+this.radius > bullet.y && this.y-this.radius < bullet.y+bullet.height){
                hitby=true;
            }
            return hitby;
    }
...
var final=new Score(200,400,"white"); 
var count=-1;
var temp_score;
...
function updategame(){ 
    ...
    ctx.drawImage(back,0,0,480,480);
    count+=1;
    start=true;
    ...
    for ( var i = 0; i<bullets.length; i++){
      ...
      if(count%10==0){
      bullets.push(new Bullet(cannon.x+(cannon.width/2)-6,cannon.y-8,17,20));
      }
